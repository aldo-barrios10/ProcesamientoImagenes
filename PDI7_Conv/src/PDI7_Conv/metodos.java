...
public class metodos extends PDI7_Conv{
    ...
    public static void conv(String filepath, int [][] matriz, String titulo) throws IOException{
            File file = new File(filepath);
            BufferedImage bufferedImage = ImageIO.read(file);
                    ...
                    for(int a=-1;a<2;a++){
                        for(int b=-1;b<2;b++){
                            ...
                            if(va<0||vb<0 || va>y-1||vb>x-1){
                                ...
                                NR=NR+ (c.getRed()*matriz[a+1][b+1]);           
                                NG=NG +(c.getGreen()*matriz[a+1][b+1]);
                                NB=NB+(c.getBlue()*matriz[a+1][b+1]);
