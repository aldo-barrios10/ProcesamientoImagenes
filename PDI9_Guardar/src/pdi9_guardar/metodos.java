...
public class metodos extends PDI9_Guardar{
    ...
    public static void ventana_aux(BufferedImage image, String titulo){
        ...
        JMenuBar bar_menu = new javax.swing.JMenuBar();
        JMenu menu = new javax.swing.JMenu();
        JMenuItem menu_archivo = new javax.swing.JMenuItem();
    ...
    public static void conv(String filepath, int [][] matriz, String titulo, int tamano) throws IOException{
            File file = new File(filepath);
        ...
        switch (tamano) {
            ...
            for(int i=1;i<=y;i++){
                for(int j=1;j<=x;j++){
                    NR=0;NG=0;NB=0;
                    for(int a=rec;a<tamano+rec;a++){
                        for(int b=rec;b<tamano+rec;b++){
                            int va=a+i-1;
                            int vb=b+j-1;
                            
                            if(va<0||vb<0 || va>y-1||vb>x-1){
                            }
                            else{
                                int f = bufferedImage.getRGB(j-1+b, i-1+a);
                                Color c = new Color(f);

                                NR=NR+ (c.getRed()*matriz[a-rec][b-rec]);           
                                NG=NG +(c.getGreen()*matriz[a-rec][b-rec]);
                                NB=NB+(c.getBlue()*matriz[a-rec][b-rec]);
                    ...
                    Color newColor = new Color(NR, NG, NB);
                    int rgb = newColor.getRGB();
                    bufferedImage2.setRGB(j-1,i-1,rgb);
                }
            }       
            ventana_aux(bufferedImage2, titulo);
        }   
}
